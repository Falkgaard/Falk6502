...
namespace cpu {
	...
	struct cpu_t {
		...
		inline auto
		read_flag( cpu_flag_e const f ) noexcept -> bool
		{
			return (P >> f) & 1U;
		}
	...
	update() noexcept
	{
		...
		for ( u8 row=0; row<0x10; ++row ) {
			...
			for ( u8 col=0; col<0x10; ++col ) {
				auto const curr_byte = ram_start[curr_addr++];
				std::printf( "\e[1;%dm%02" PRIX8 "%s", (curr_byte? 32:31), curr_byte, col==0xF?"\e[0m":" " );
			}
...
main( int const argc, char const *const argv[] )
{
	...
	for ( u8 y=0; y<pg_y; ++y )
		for ( u8 x=0; x<pg_x; ++x )
			hex_page_displays[y*pg_x+x] = ram_page_hex_display_t { system.RAM, pg_no++, u16(x*56U+85U), u16(y*20U+1U) };
